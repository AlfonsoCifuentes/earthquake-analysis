...
st.markdown("""
This dashboard allows you to explore seismic data for a full month.
...
def load_data():
    try:
        # Load earthquake data from USGS official API endpoint
...
try:
    ...
    if df is not None and not df.empty:
        ...
        if len(date_range) == 2:
            ...
            # Filter the dataframe (now both are the same type)
        ...
        if len(filtered_df) == 0:
            ...
            with main_tabs[0]:
                ...
                # Magnitudes and depths distribution
            ...
            with main_tabs[1]:
                geo_tabs = st.tabs(["Events Map", "Heat Map", "Cluster Analysis"])
                ...
                with geo_tabs[1]:
                    ...
                    # Show significant events in table form instead of additional map
                    st.subheader("Significant Events (Magnitude ≥ 4.0)")
                ...
                with geo_tabs[2]:
                    ...
                    st.markdown("""
                    This analysis identifies groups of earthquakes that might be geographically related.
                    It uses the DBSCAN algorithm which groups events based on their spatial proximity.
                    """)
                    
                    # Prepare data for clustering
                    if len(filtered_df) > 10:  # Ensure there's enough data
                        ...
                        # Create a column to map cluster to string for better visualization
                        filtered_df['cluster_str'] = filtered_df['cluster'].apply(
                            lambda x: f'Cluster {x}' if x >= 0 else 'No Cluster'
                        )
                        
                        # Use scatter_geo for the clusters map
                                ...
                                if cluster_options:
                                    ...
                                    if not cluster_data.empty:
                                        ...
                                        **Cluster Interpretation:**
                                        ...
                                        or patterns of seismic activity in a particular region.
...
This dashboard displays seismic data from approximately one month of activity.
